         ...
         <div class="movie-browse-agile">
            ...
            <div class="browse-agile-w3ls general-w3ls">
               <div class="tittle-head">
                  <h4 class="latest-text ml0">Popular</h4>
                                    ...
                                    <div class="w3-agileits-news-left">
                                       <div class="col-sm-3 wthree-news-img">
                                          <a href="{{ url('/video-details/'.@$data->id.'/'.str_slug($data->title)) }}" class="hvr-shutter-out-horizontal">
                                            @if(!empty($data->image))
                                            <img src="{{ url('uploads/'.@$data->image)}}" title="album-name" class="img-responsive img-trending" alt=" "  />
                                            @else
                                       ...
                                       <div class="col-sm-9 wthree-news-info">
                                          <h3><a href="{{ url('/video-details/'.@$data->id.'/'.str_slug($data->title)) }}">{{ str_limit(@$data->title, 25, '...') }}</a></h3>
                                          <ul style="margin-bottom: 2%;">
                                            <li><i class="fa fa-user" aria-hidden="true"></i>
                                               <a href="{{ url('front_channel/'.@$data->channel->id)}}">
                                                  {{ @$data->channel->channel }}
                                               </a></li>
                                             <li><i class="fa fa-clock-o" aria-hidden="true"></i>{{ date_format(@$data->created_at,"M d,Y") }}</li>
                                             <li><i class="fa fa-eye" aria-hidden="true"></i> 0</li>
                                          </ul>
                                          <p>{{str_limit(@$data->description, 300, ' (.......)')}}</p>
                                       </div>
                                    ...
                                    @foreach($datas as $data)
                                       <div class="col-md-4 w3-agileits-news-left">
                                          <div class="col-sm-12 wthree-news-img">
                                             <a href="{{ url('/video-details/'.@$data->id.'/'.str_slug($data->title)) }}">
                                              @if(!empty($data->image))
